...
class AttributePropertyModel extends PropertyModel implements IPropertyModel {
	...
	public AttributePropertyModel(IModule module, Attribute attribute) {
		super (module);
	...
	public void changeProperty(int row, String value) {
		...
		IModelingSession session = getModule().getModelingSession ();
	...
	private void changePropertySetter(boolean value) {
		if (value) {
	...
	protected void changePropertyGetter(boolean value) {
		if (value) {
	...
	private boolean mustUseWrapper(Attribute theAttribute) {
    ...
    private boolean updateAccessors(IModelingSession session) throws ExtensionNotFoundException, CustomException {
		if (!this.attribute.isStereotyped(IJavaDesignerPeerModule.MODULE_NAME, JavaDesignerStereotypes.JAVAATTRIBUTEPROPERTY)) {
			IModuleUserConfiguration javaConfig = getModule ().getConfiguration ();
