...
public class InterfaceImplementer {
     ...
     */
    public InterfaceImplementer(IModelingSession session) {
    ...
    public boolean updateImplementingClassifiers(Interface theInterface) {
        boolean result = false;

        for (InterfaceRealization theRealization : theInterface
                .getImplementedLink()) {
            NameSpace theNameSpace = theRealization.getImplementer();
            if (theNameSpace != null && theNameSpace instanceof Classifier) {
                boolean newResult = implementInterface(
                        (Classifier) theNameSpace, theInterface);
     ...
     * Interfaces.
     * @param futureOperationOwner
     * The Class to create the Operations in.
     */
    public boolean implementInterfaces(Classifier futureOperationOwner) {
        boolean ret = true;
        // Take all Interfaces
        for (Iterator<InterfaceRealization> iter = futureOperationOwner
                .getRealized().iterator(); iter.hasNext();) {
     ...
     * The Class to create the Operations in.
     * @param itf
     * The Interface containing the Operations to redefine.
     */
    public boolean implementInterface(Classifier futureOperationOwner, Interface itf) {
        try {
            // Take all Operations
     ...
     * @param futureOperationOwner
     * The Class to create the Operations in.
     * @param redefinedOperation
     * The Operation to redefine
     */
    public void implementAnOperation(Classifier futureOperationOwner, Operation redefinedOperation) {
        // Check if an existing operation matches
        Operation implementingOperation = getImplementingOperation(
                redefinedOperation, futureOperationOwner);
        if (implementingOperation == null) {
            ...
            if (!futureOperationOwner.isIsAbstract()) {
                implementingOperation = this.model.createOperation(
                        redefinedOperation.getName(), futureOperationOwner);
     ...
     */
    public void implementAnOperation(Operation predefop) {
        try {
     ...
     * The Class to create the Operations in.
     */
    public boolean unImplementInterfaces(Classifier current) {
        boolean res = false;

        // Take all Operations
        for (Operation op : current.getOwnedOperation()) {
            // Take all Interfaces
            for (Iterator<InterfaceRealization> iter = current.getRealized()
    ...
    private boolean contentEquals(Operation op1, Operation op2) {
        boolean found = true;
        ...
        if (contentEquals(op1.getReturn(), op2.getReturn())) {
            ...
            if (op1.getIO().size() == op2.getIO().size()) {
                ...
                for (Iterator<Parameter> iterator2 = op1.getIO()
                        .iterator(); iterator2.hasNext() && found;) {
                ...
                if (op1.getThrown().size() == op2.getThrown().size()) {
                    Iterator<RaisedException> itOldExcept = op2.getThrown()
                            .iterator();
                    for (Iterator<RaisedException> itNewExcept = op1
                            .getThrown().iterator(); itNewExcept.hasNext()
                            && found;) {
     ...
     */
    private Operation getImplementingOperation(Operation operationToMatch, Classifier potentialParent) {
     ...
     */
    private Operation getSuperOperation(Operation operationToMatch, Interface potentialParent) {
     ...
     */
    private boolean contentEquals(Parameter p1, Parameter p2) {
     ...
     * @param dest
     * The owner of the Operation to create.
     */
    private void copyOperationContent(Operation orig, Operation dest) {
    ...
    private void copyParameters(Operation orig, Operation dest) {
    ...
    private void copyExceptions(Operation orig, Operation dest) {
        // Update parameters
    ...
    private void copyStereotypes(ModelElement orig, ModelElement dest) {
        {
    ...
    private void copyTaggedValues(ModelElement orig, ModelElement dest) {
    ...
    private void copyTagParameters(TaggedValue orig, TaggedValue dest) {
    ...
    private void copyTagQualifier(TaggedValue orig, TaggedValue dest) {
    ...
    private void copyNotes(ModelElement orig, ModelElement dest) {
    ...
    private void copyParameterContent(Parameter orig, Parameter dest) {
    ...
    private void copyImports(Operation orig, Operation dest) {
        // ElementImports
     ...
     */
    private List<Classifier> getImplementers(Interface pinterf) {
    ...
    private List<Operation> getImplementedOperations(Classifier clsfr, Interface interf) {
     ...
     * more implemented.
     * @param operationsOwner
     * The classifier owning the operations.
     */
    public boolean updateRedefineLinks(Classifier operationsOwner) {
        boolean hasDoneWork = false;

        ArrayList<Interface> implementedInterfaces = new ArrayList<>();
        // Take all Interfaces
        for (Iterator<InterfaceRealization> iter = operationsOwner
                .getRealized().iterator(); iter.hasNext();) {
            Interface itf = iter.next().getImplemented();
        ...
        for (Operation operation : operationsOwner.getOwnedOperation()) {
            ...
            if (redefinedOperation != null) {
                Classifier redefinedOperationOwner = redefinedOperation
                        .getOwner();
                if ((redefinedOperationOwner instanceof Interface)
                        && !implementedInterfaces.contains(redefinedOperation)) {
     ...
     * @param porig
     * The Class where the operations are
     * @param pdest
     * The Class where to operation should be copied to
     * @param pinterf
     * The Interface to filter operations to move
     */
    public void moveOperations(Classifier porig, Classifier pdest, Interface pinterf) {
