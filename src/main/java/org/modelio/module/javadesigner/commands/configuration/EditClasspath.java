    ...
    public void actionPerformed(List<MObject> selectedElements, IModule module) {
        ...
        if (rw.open() == 0) {
            ...
            try (ITransaction t = session.createTransaction("update classpath")) {
                ...
                // Transaction alreay rollbacked
            }
        }
    }

    private void updateClassPath(List<File> classpath, IModule module) {
        ...
        if (buffer.length () != 0) {
            module.getConfiguration ().setParameterValue (JavaDesignerParameters.ACCESSIBLECLASSES, buffer.substring (0, buffer.length () - 1));
        } else {
            module.getConfiguration ().setParameterValue (JavaDesignerParameters.ACCESSIBLECLASSES, ""); //$NON-NLS-1$
        }
    }
