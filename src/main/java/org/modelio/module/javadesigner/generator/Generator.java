...
public class Generator {
    ...
    public Generator(Collection<NameSpace> elements, IModule module) {
        this.module = module;
        this.elementsToGenerate = elements;
    }

    public void generate(ReportModel report) {
        ...
        if (!cancel) {
            ...
            try (ITransaction transaction = session.createTransaction (Messages.getString ("Info.Session.Generate"))) {
                ...
                String retrieveDefaultBehavior = this.module.getConfiguration ().getParameterValue (JavaDesignerParameters.RETRIEVEDEFAULTBEHAVIOUR);
                ReverseMode mode = ReverseMode.Ask;
                if (retrieveDefaultBehavior.contentEquals (JavaDesignerParameters.RetrieveMode.Retrieve.toString ())) {
                    mode = ReverseMode.Retrieve;
                } else if (retrieveDefaultBehavior.contentEquals (JavaDesignerParameters.RetrieveMode.Keep.toString ())) {
                    mode = ReverseMode.Keep;
                }

                // Remove all packages from the element list to avoid reversing classes after choosing "keep" in the
                ...
                try {
                    reversor.update (this.elementsToGenerate, mode, EditorManager.getInstance ());
                ...
                if (cancel) {
                    ...
                    // the commit
                    IModuleSession javaDesignerSession = this.module.getSession ();
                    IModelChangeHandler handler = null;
                    if (javaDesignerSession != null &&
                            javaDesignerSession instanceof ISessionWithHandler) {
