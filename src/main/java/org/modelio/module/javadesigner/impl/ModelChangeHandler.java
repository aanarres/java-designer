...
public class ModelChangeHandler implements IModelChangeHandler {
    ...
    public ModelChangeHandler(IModule javaDevModule) {
        this.javaDevModule = javaDevModule;
        IModelingSession session = javaDevModule.getModelingSession ();
        this.createVisitor = new CreatedElementVisitor (session);
     ...
     */
    @Override
    public void handleModelChange(IModelingSession coreSession, IModelChangeEvent event) {
        IModelingSession session = this.javaDevModule.getModelingSession ();

        try (ITransaction t = session.createTransaction("handle model change")) {
            IModuleUserConfiguration javaConfig = this.javaDevModule.getConfiguration ();
            ...
            if (deleteEvents.size() > 0) {
                for (IElementDeletedEvent deletedEvent : deleteEvents) {
                ...
                if ((createdElement != null) && createdElement.getStatus ().isModifiable ()) {
                    if (createdElement instanceof TaggedValue) {
                        ...
                        hasDoneWork = hasDoneWork ||
                                this.createVisitor.hasDoneWork;
                    }
                }
            }
            ...
            if (updatedElements.size () > 0) {
                ...
                for (MObject updatedElement : updatedElements) {
                    ...
                    if ((updatedElement != null) && updatedElement.getStatus ().isModifiable ()) {
                        ...
                        // Check if the visitor has created something
                        hasDoneWork = hasDoneWork ||
                                this.updateVisitor.hasDoneWork;
                    }
                }
            }
            t.commit();
     ...
     */
    boolean handleUpdate(Attribute theAttribute) throws ExtensionNotFoundException, CustomException, ElementNotUniqueException {
     ...
     * @throws ElementNotUniqueException
     */
    boolean handleUpdate(AssociationEnd theAssociationEnd) throws ExtensionNotFoundException, CustomException, ElementNotUniqueException {
        boolean ret = true;
     ...
     */
    boolean isAccessorGenerated(Feature theFeature) {
    ...
    private class CreatedElementVisitor extends DefaultModelVisitor {
        ...
        public CreatedElementVisitor(IModelingSession session) {
        ...
        public Object visitClass(Class theClass) {
            ...
            if (JavaDesignerUtils.isJavaElement (owner)) {
                if (!JavaDesignerUtils.isJavaElement (theClass)) {
                    theClass.getExtension().add (this.session.getMetamodelExtensions ().getStereotype (IJavaDesignerPeerModule.MODULE_NAME, JavaDesignerStereotypes.JAVACLASS, theClass.getMClass ()));
                    this.hasDoneWork = true;
                }
            }
            return null;
        ...
        public Object visitDataType(DataType theDataType) {
            ...
            if (JavaDesignerUtils.isJavaElement (owner)) {
                if (!JavaDesignerUtils.isJavaElement (theDataType)) {
                    theDataType.getExtension().add (this.session.getMetamodelExtensions ().getStereotype (IJavaDesignerPeerModule.MODULE_NAME, JavaDesignerStereotypes.JAVADATATYPE, theDataType.getMClass ()));
                    this.hasDoneWork = true;
                }
            }
            return null;
        ...
        public Object visitEnumeration(Enumeration theEnumeration) {
            ...
            if (JavaDesignerUtils.isJavaElement (owner)) {
                if (!JavaDesignerUtils.isJavaElement (theEnumeration)) {
                    theEnumeration.getExtension().add (this.session.getMetamodelExtensions ().getStereotype (IJavaDesignerPeerModule.MODULE_NAME, JavaDesignerStereotypes.JAVAENUMERATION, theEnumeration.getMClass ()));
                    this.hasDoneWork = true;
                }
            }
            return null;
        ...
        public Object visitInterface(Interface theInterface) {
            ...
            if (JavaDesignerUtils.isJavaElement (owner)) {
                if (!JavaDesignerUtils.isJavaElement (theInterface)) {
                    theInterface.getExtension().add (this.session.getMetamodelExtensions ().getStereotype (IJavaDesignerPeerModule.MODULE_NAME, JavaDesignerStereotypes.JAVAINTERFACE, theInterface.getMClass ()));
                    this.hasDoneWork = true;
                }
            }
            return null;
        ...
        public Object visitPackage(Package thePackage) {
            ...
            if (JavaDesignerUtils.isJavaElement (owner)) {
                if (!JavaDesignerUtils.isJavaElement (thePackage)) {
                    thePackage.getExtension().add (this.session.getMetamodelExtensions ().getStereotype (IJavaDesignerPeerModule.MODULE_NAME, JavaDesignerStereotypes.JAVAPACKAGE, thePackage.getMClass ()));
                    this.hasDoneWork = true;
                }
            }
            return null;
    ...
    private class UpdateElementVisitor extends DefaultModelVisitor {
        ...
        private int getAutomationBehavior(Interface theInterface) {
            int result = 0;
            MessageDialog dialog = new MessageDialog (Display.getDefault ().getActiveShell (), Messages.getString ("Gui.AutomaticInterfaceImplementation.Title"), null, Messages.getString ("Gui.AutomaticInterfaceImplementation.Message", JavaDesignerUtils.getFullJavaName (ModelChangeHandler.this.javaDevModule.getModelingSession(), theInterface)), MessageDialog.QUESTION, new String[] { //$NON-NLS-1$ //$NON-NLS-2$
