...
public class BinaryAnalyzer {
	...
	BinaryAnalyzer(final List<File> aClasspath, IReportWriter report) {
	    ...
	    for (File file : aClasspath) {
	        try {
	            ...
	            JavaDesignerModule.logService.warning(e);
	 ...
	 * classpath.
	 * 
	 * @param aFullQualifiedName
	 *            full qualified name of the new element
	 * @return true a new structural has been added
	 * @throws NameCollisionException in case of name collision
	 */
	boolean addBinaryEntryToStructuralModel(String aFullQualifiedName,
			PackageDef root) throws NameCollisionException {
		...
		do {
			try {
                ...
                JavaDesignerModule.logService.info(e);
                ...
                // register what is available on the class
                JavaDesignerModule.logService.warning(e.toString());
                JavaDesignerModule.logService.info(e);
	...
	private String getMessage(ClassifierDefKind kind) {
    ...
    private ClassifierDef defineClassDef(final String aLoadableClassName, final PackageDef root) {
		...
		while (dolidx != -1) {
			...
			par = StructuralModelUtils.addClass(clazz, par);
			beg = dolidx+1;
			dolidx = classparts.indexOf(beg, '$');
		}
		newClass = StructuralModelUtils.addClass(classparts.substring(beg, classparts.length()), par);
		return newClass;
	}

	private ClassifierDef defineClassDef(final Class<?> aClass, final PackageDef root, boolean checkClassPath)
		...
		if (!checkClassPath || (checkClassPath && isInClassPath(aClass))) {
			...
			newClass = StructuralModelUtils.addClass(aClass.getSimpleName(), parent);
			// Set kind of classifierDef
			if (aClass.isAnnotation()) {
				newClass.setKind(ClassifierDefKind.CLASS);
			} else if (aClass.isInterface()) {
				newClass.setKind(ClassifierDefKind.INTERFACE);
			} else if (aClass.isEnum()) {
				newClass.setKind(ClassifierDefKind.ENUMERATION);
			} else if (aClass.isPrimitive()) {
				newClass.setKind(ClassifierDefKind.DATATYPE);
			} else {
				newClass.setKind(ClassifierDefKind.CLASS);
			}
	 ...
	 */
	boolean isBinaryPackage(String aFullQualifiedName) {
	 ...
	 */
	private boolean isInClassPath(Class<?> aClass) {
