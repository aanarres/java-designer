...
class TypeAstServices {
	 ...
	 * 
	 * @param type
	 *            TYPE ast node that may carry an array.
	 ...
	 */
	public static boolean isArrayAnnotated(ASTTree type) {
		boolean ret = false;
	 ...
	 */
	public static boolean isGenericType(ASTTree type) {
		assert (type.getType() == JavaParser.TYPE) : "Internal error: unexpected input ast node type";
		return getTypeArgs(type) != null;
	}

	public static boolean isFullQualifiedNamed(ASTTree type) {
		assert (type.getType() == JavaParser.TYPE) : "Internal error: unexpected input ast node type";
		return type.getChild(0).getType() == JavaParser.DOT;
	}

	public static List<String> getBoundTypesSourceCode(ASTTree genericType) {
	...
	static ASTTree getTypeArgs(ASTTree type) {
	 ...
	 */
	public static boolean isUMLMappable(ASTTree type) {
		assert (type.getType() == JavaParser.TYPE) : "Internal error: unexpected input ast node type";
	...
	public static boolean isWildCard(ASTTree type) {
		assert (type.getType() == JavaParser.TYPE) : "Internal error: unexpected input ast node type";
		int childType = type.getChild(0).getType();
		return childType == JavaParser.WILDCARD;
	}

	public static String getMultiplicityMax(ASTTree type) {
		assert (type.getType() == JavaParser.TYPE) : "Internal error: unexpected input ast node type";
		String multmax;
